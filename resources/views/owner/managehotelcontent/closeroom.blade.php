...
<body>
    ...
    <section class="container p-4">
        ...
        <h1 class="text-center mt-2">Đóng Cửa {{ $room->r_name }}</h1>
        <hr>
        <div class="d-flex gap-2">
            ...
            @else
                @php
                    $ngaydong = date('d/m/Y', strtotime($room->r_dateclose));
                    $ngaymo = date('d/m/Y', strtotime($room->r_dateopen));
                @endphp
                <h4 class="text-danger">Phòng đóng ({{ $ngaydong . ' tới ' . $ngaymo }})</h4>
            @endif
        </div>
        <form
            @if ($room->r_isclose == 0) action="{{ route('closeroom', ['rid' => $room->r_id]) }}"
        @else
        action="{{ route('openroom', ['rid' => $room->r_id]) }}" @endif
            ...
            <div class="col-md-8 col-12 d-flex align-items-end gap-2">
                <div class="flex-fill"><label for="ngayBD">ngày đóng cửa</label>
                    <input id="inputCin" type="date" name="dayclose" class="form-control border-secondary w-100"
                        @if ($room->r_isclose == 1) disabled @endif>
                </div>
